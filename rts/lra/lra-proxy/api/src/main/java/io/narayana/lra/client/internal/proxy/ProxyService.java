    ...
    public URI joinLRA(LRAProxyParticipant participant, URI lraId, Long timelimit, ChronoUnit unit) {
        ...
        try {
            ...
            if (response.getStatus() == Response.Status.PRECONDITION_FAILED.getStatusCode()) {
                throwNewJoinLRAException(lraId, response.getStatus(), "Too late to join with this LRA", null);
            } else if (response.getStatus() != Response.Status.OK.getStatusCode()) {
                throwNewJoinLRAException(lraId, response.getStatus(), "Unable to join with this LRA", null);
            }

            return new URI(response.readEntity(String.class));
        } catch (Exception e) {
            throwNewJoinLRAException(lraId, 0, "Exception whilst joining with this LRA", e);
        }

        return null;
    }

    private void throwNewJoinLRAException(URI lraId, int status, String s, Throwable e) {
        throw new WebApplicationException(e, Response.status(status).entity(lraId + ": " + s).build());
    }
